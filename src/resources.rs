...
async fn add_resource(
    ...
    let auth = req
        ...
        .to_str()
        .unwrap();
    if (target_type.target_type == models::TargetType::Bot
        && data.database.authorize_bot(id, auth).await)
        || (target_type.target_type == models::TargetType::Server
            && data.database.authorize_server(id, auth).await)
    {
        // Add resource
...
async fn delete_resource(
    ...
    let auth = req
        ...
        .to_str()
        .unwrap();
    if (query.target_type == models::TargetType::Bot && data.database.authorize_bot(id, auth).await)
        || (query.target_type == models::TargetType::Server
            && data.database.authorize_server(id, auth).await)
    {
        // Get resource owner
        ...
        let resource_owned = data
            .database
            .resource_exists(resource_id, id, query.target_type)
            .await;
